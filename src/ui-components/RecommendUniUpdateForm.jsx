...
export default function RecommendUniUpdateForm(props) {
  const {
    id: idProp,
    recommendUni: recommendUniModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = recommendUniRecord
      ? { ...initialValues, ...recommendUniRecord }
      : initialValues;
  ...
  const [recommendUniRecord, setRecommendUniRecord] = React.useState(
    recommendUniModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(RecommendUni, idProp)
        : recommendUniModelProp;
      setRecommendUniRecord(record);
    };
    queryData();
  }, [idProp, recommendUniModelProp]);
  React.useEffect(resetStateValues, [recommendUniRecord]);
  const validations = {
    weather: [],
    region: [],
    country: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            RecommendUni.copyOf(recommendUniRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "RecommendUniUpdateForm")}
      {...rest}
    >
      <TextField
        label="Weather"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || recommendUniModelProp)}
